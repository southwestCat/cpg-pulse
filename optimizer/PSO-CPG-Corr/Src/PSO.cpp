...
void PSO::initParams() {
  ...
  //   const float obj = f.calcFitness(X[i]);
  //   pbest[i] = X[i];
  //   pbest_obj[i] = obj;
  // }
...
void PSO::update() {
  for (int it = 0; it < nIter; it++) {
    ...
    checkGBest();
    if (progTerminate) break;

    cout << it << " -> | " << gbest_obj << " | " << gbest[0] << ", " << gbest[1]
         << ", " << gbest[2] << ", " << gbest[3] << ", " << gbest[4] << ", "
         << gbest[5] << endl;

    if ((it + 1) % 10 == 0) updateAnswer();
  }
}
...
void PSO::updateParticles() {
  ...
  auto f1 = [&](int b) {
    for (size_t i = 0; i < nTasks; i++) {
      V[i + b] = w * V[i + b] + c1 * r1 * (pbest[i + b] - X[i + b]) +
                 c2 * r2 * (gbest - X[i + b]);
      X[i + b] = X[i + b] + V[i + b];
    }
  };
