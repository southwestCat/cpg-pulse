...
Matsuoka2Neu::Matsuoka2Neu(CPGPattern type, float dt)
{
    ...
    rk.func = [&](const RK::state_type &y, RK::state_type &args) -> RK::state_type
    {
        float u1 = y[0];
        float v1 = y[1];
        float u2 = y[2];
...
void Matsuoka2Neu::setParams(float Tu, float Tv, float s0, float b, float wfe, float feed1, float feed2, float K_feed1, float K_feed2)
{
    this->Tu = Tu;
    this->Tv = Tv;
    this->s0 = s0;
...
void Matsuoka2Neu::integrate()
{
    rk.integrate(dt, args);
}
...
float Matsuoka2Neu::getYAbsMax(float maxtime)
{
    ...
    for (auto state : rk.states)
    {
        const float u1 = state[0];
        const float u2 = state[2];
        const float y1 = std::max(0.f, u1);
        const float y2 = std::max(0.f, u2);
        const float y = y2 - y1;
        if (abs(y) > ymax)
            ymax = abs(y);
    }
