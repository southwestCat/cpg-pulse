...
void Nao::logInfo() {
    float comx = theCPGGeneratorOutput->comx;
    float comy = theCPGGeneratorOutput->comy;
...
void Nao::readSensors()
{
    ...
    theFsrSensorData->totals[Legs::left] = fsrData(0, 0) + fsrData(0, 1) + fsrData(0, 2) + fsrData(0, 3);
    theFsrSensorData->totals[Legs::right] = fsrData(1, 0) + fsrData(1, 1) + fsrData(1, 2) + fsrData(1, 3);
}
...
Vector3f Nao::getValuesOfInertialUnit()
{
    ...
    // printf("roll/pitch/yaw: = [%3.2f %3.2f %3.2f]deg\n", toDegrees(rpy[0]), toDegrees(rpy[1]), toDegrees(rpy[2]));
...
void Nao::testMotor()
{
    ...
    {
        if (id == Joints::lHand || id == Joints::rHand)
            continue;
        for (int j = 0; j < 2; j++)
        {
...
void Nao::initPosture()
{
    ...
    {
        t += dt;
        float target = initHeight - (t / duringT) * (initHeight - MotionConfig::hipHeight);
        ...
        bool isPossible = InverseKinematic::calcLegJoints(targetL, targetR, Vector2f::Zero(), *theJointRequest, *theRobotDimensions);
        if (isPossible)
        {
            setJoints(*theJointRequest);
        }
...
void Nao::resetSimulation(){
    theZMPData->reset();
    generator->reset();

    this->simulationReset();
    this->simulationResetPhysics();
