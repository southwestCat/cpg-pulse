...
Queue::Queue(size_t size) { reset(size); }

Queue::~Queue() { std::vector<float>().swap(_d); }
...
float Queue::sum() {
  float s = 0.f;
  for_each(_d.cbegin(), _d.cend(), [&s](float n) { s += n; });
  return s;
}

float Queue::average() { return sum() / _size; }

float Queue::variance() {
  float a = average();
  float svar = 0.f;
  for_each(_d.cbegin(), _d.cend(), [&](float n) { svar += (n - a) * (n - a); });
  return svar / _size;
}

int Queue::sign() {
  int s = 0;
  for_each(_d.cbegin(), _d.cend(), [&](float n) { s += n > 0 ? 1 : -1; });
  return s;
}
